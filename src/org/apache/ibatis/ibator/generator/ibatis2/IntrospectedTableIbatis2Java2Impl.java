...
public class IntrospectedTableIbatis2Java2Impl extends IntrospectedTable {
	...
	protected void calculateDAOGenerators(List<String> warnings,
		...
		String type = ibatorContext.getDaoGeneratorConfiguration()
		...
		if ("IBATIS".equalsIgnoreCase(type)) { //$NON-NLS-1$
			...
			javaGenerator = (AbstractJavaGenerator) IbatorObjectFactory
		...
		for (AbstractJavaGenerator javaGenerator : javaModelGenerators) {
			...
			for (CompilationUnit compilationUnit : compilationUnits) {
				...
				if (StringUtility.stringHasValue(ibatorContext
						.getJavaModelGeneratorConfiguration().getProperty(
								"suffix"))) {
					gjf.setSufFix(ibatorContext
							.getJavaModelGeneratorConfiguration().getProperty(
									"suffix"));
				}
				if (StringUtility.stringHasValue(ibatorContext
	...
	public List<GeneratedXmlFile> getGeneratedXmlFiles() {
		...
		GeneratedXmlFile gxf = new GeneratedXmlFile(document,
				getIbatis2SqlMapFileName(), getIbatis2SqlMapPackage(),
